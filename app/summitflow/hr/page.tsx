...
import { StatCard } from '@/components/summitflow/StatCard';
import { ActivityFeed } from '@/components/summitflow/ActivityFeed';
import { getCurrentProfile } from '@/lib/summitflow/session';
import { MOCK_ACTIVITY, MOCK_MILEAGE, MOCK_PROFILES, MOCK_SITES } from '@/lib/summitflow/mock-data';
import { shifts } from '@/lib/summitflow/shift-store';
import { isOpenShift } from '@/lib/summitflow/shift-utils';
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6 sm:mb-8">
        <StatCard label="Open shifts" value={openCount} accent="#f59e0b" sub="Awaiting technicians" />
        ...
        <Link href={`${OPS_BASE}/hr/approvals`} className="ops-card p-4 hover:border-[var(--ops-blue)] transition-colors group">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs ops-text-muted">Mileage &amp; job logs</div>
            </div>
          </div>
        </Link>
